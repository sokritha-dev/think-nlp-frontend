...
export const SentimentCharts = ({ sentimentResults }: SentimentChartsProps) => {
  ...
  const computeAverage = (positive: number, negative: number) => {
        ...
        sentimentResults && (<>
            ...
            <div className="md:w-1/2">
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-2">
                  {getSentimentIcon(computeAverage(sentimentResults.overall.positive, sentimentResults.overall.negative))}
                  <h2 className="text-lg font-semibold">
                    {getSentimentText(computeAverage(sentimentResults.overall.positive, sentimentResults.overall.negative))} Overall Sentiment
                  </h2>
                </div>
                <p className="text-sm text-gray-600 mb-4">
                  The average sentiment across all reviews leans {getSentimentText(computeAverage(sentimentResults.overall.positive, sentimentResults.overall.negative)).toLowerCase()},
          ...
          <div>
            ...
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={sentimentResults.per_topic}
                  layout="vertical"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
              {sentimentResults.per_topic.map((topic, idx) => (
                <div key={idx} className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
                  <div className="flex items-center gap-2 mb-2">
                    {getSentimentIcon(computeAverage(topic.positive, topic.negative))}
                    <h4 className="font-medium text-sm truncate">{topic.label}</h4>
                  </div>
